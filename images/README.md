...
| `ubuntu-24.04-desktop/smoke.py` | Candidate qualification for X11, the Driver session socket and user unit, native `list_apps`, a whole-desktop screenshot that is not a flat frame, disabled nesting, and owned-resource cleanup. |
...
The daemon serves with `--no-overlay`. The Driver's agent-cursor overlay maps a
full-screen, override-redirect, depth-24 window with save-unders; while that
window is mapped, every X root-window read on this GNOME/Xorg session returns
the frame from the moment the overlay appeared instead of live pixels. The user
unit starts before Mutter's first composite, so that frozen frame is solid
black: `get_desktop_state` and the x0vncserver view both show a black 1280x800
screen for the life of the guest, while per-window captures keep working
because they read redirected window pixmaps. Unmapping the overlay restores
live pixels within a frame. Reproduced on Driver 0.28.1 and 0.28.2, so this is
an upstream defect and not a pin-bump fix; `--no-overlay` is upstream's own
documented switch for the daemon that owns the overlay.

The cost is the synthetic agent cursor: pointer actions no longer draw a
session-coloured cursor for a human watching over VNC. Input itself is
unaffected — clicks, typing, and window captures behave exactly as before.
Anyone who re-enables the overlay, by dropping the flag or by starting a second
daemon without it, reintroduces the black desktop for that guest; `build.py`
refuses a wrapper that serves without `--no-overlay`, and the desktop smoke
fails a flat screenshot.
